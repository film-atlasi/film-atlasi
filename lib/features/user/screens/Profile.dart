...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  bool isLoading = true;
  late String userUid;
  final followServices = FollowServices(); // Add this line to define followServices
    ...
    try {
      ...
      if (currentUser != null) {
        final snapshot = await FirebaseFirestore.instance
            ...
            final followers = snapshot.data()?['followers'] ?? 0; // 🔥 Takipçi sayısını al
            final following = snapshot.data()?['following'] ?? 0; // 🔥 Takip edilenleri al
        ...
        setState(() {
          ...
          userData?['followers'] = followers;  // 🔥 Takipçi sayısını kaydet
          userData?['following'] = following;  // 🔥 Takip edilenleri kaydet
          isLoading = false;
        });
      }
    } catch (e) {
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _buildStatItem(userData?['postCount'].toString() ?? "0", "Film"),
             _buildStatItem(userData?['following'].toString() ?? "0", "Takip Edilen"),
             _buildStatItem(userData?['followers'].toString() ?? "0", "Takipçi"),
            ],
          ),
        ],
      ),
 ...
 Widget _buildStatItem(String value, String label) {
    ...
    return showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return FollowListWidget(userUid: userUid, isFollowers: true);
      },
    );
  }
  ...
  Future<dynamic> buildTakipEdilenler() {
    return showModalBottomSheet(
      context: context,
      builder: (BuildContext context) {
        return FollowListWidget(userUid: userUid, isFollowers: false);
      },
    );
  }
